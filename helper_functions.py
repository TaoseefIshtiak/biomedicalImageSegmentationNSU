...
def mean_iou(y_true, y_pred):
    ...
    for t in np.arange(0.5, 1.0, 0.05):
        ...
        score, up_opt = tf.metrics.mean_iou(y_true, y_pred_, 2)# 2 is the classification number here
        K.get_session().run(tf.local_variables_initializer())#a session against each model
        with tf.control_dependencies([up_opt]):#control dependencies runs chunk of the operation that increments the confusion matrix
            score = tf.identity(score) #replicates the tensor object a mechanism to duplicate the tensor inputs
        prec.append(score)# precision miou score lists being appended with each iteration
    return K.mean(K.stack(prec), axis=0) #keras object mean calculation
...
def dice_coef_loss(y_true, y_pred): #similar to accuracy vs classification erro
    return 1. - dice_coef(y_true, y_pred)

def bce_dice_loss(y_true, y_pred): # similar to weighted cross entropy but also weights up the negative examples
